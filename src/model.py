...
from src.dataset import load_dataset, configure_for_performance
...
def build_train_model(filepath, filter_size, img_size, batch_size):
    train_dataset, train_size = load_dataset(f"{filepath}/train")
    train_dataset = configure_for_performance(train_dataset, batch_size)
    valid_dataset, valid_size = load_dataset(f"{filepath}/validation")
    test_dataset, test_size = load_dataset(f"{filepath}/test")
